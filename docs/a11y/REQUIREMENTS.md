...
| A11Y-05 | Text reads at 4.5:1 or better in every theme: muted text and the brand colour (option 1, decided in Batch 20: each colour keeps its hue and moves only as far as 4.6:1 on every background it sits on; the brand is a fill, `--brand`, and an ink, `--brand-ink`), colours from a Telegram theme included (D4), and the accent colours of badges and buttons, read from the stylesheet itself. axe finds nothing serious or critical in eight screens, light and dark. The year strip is no trap for the keyboard (Y1). | 1.4.3, 2.1.1, 4.1.2 | `test_the_fallback_palette_meets_wcag_aa_text_contrast`, `test_axe_finds_no_serious_or_critical_violations`, `test_muted_text_stays_readable_under_a_low_contrast_telegram_theme`, `test_the_year_strip_is_not_a_trap_for_the_keyboard`, `test_accent_text_meets_wcag_aa_on_its_own_background` |
...
| A11Y-07 | The public countdown page has a `main` landmark and a heading (visually hidden, so the page looks as before), and its image's alt text carries the countdown in the card's own words ("12 days left", "Today", "3 days ago", in English and Persian). | 1.1.1, 1.3.1 | `test_the_countdown_image_alt_text_carries_the_countdown`, `test_the_countdown_alt_also_says_today_and_past`, `test_the_countdown_page_has_a_main_landmark_and_a_heading` |
...
| A11Y-09 | The console stays clean under the production CSP: the page outside Telegram is built from nodes and stylesheet classes, with no style attributes (it used to show unstyled, with an error). | CSP, 4.1.1 | `test_opening_the_app_outside_telegram_raises_no_csp_violation` |
